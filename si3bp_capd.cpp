...
interval gAtPoint(IPoincareMap& pmPeri, double p)
{
  MapEval E = evalP(pmPeri,interval(p),ZERO,1);
  return E.image[PZ];
}

interval gPrimeOn(IPoincareMap& pmPeri, const interval& p)
{
  MapEval E = evalP(pmPeri,p,ZERO,1);
  return E.D2[0][0];
}
...
struct C1Certificate {
  bool pass=false;
  interval pRoot;
  interval gLeft,gRight,gPrime;
  IMatrix DF;
  interval lambdaU,lambdaS;

  C1Certificate() : pRoot(0.),gLeft(0.),gRight(0.),gPrime(0.),DF(2,2),lambdaU(0.),lambdaS(0.) {}
};
...
C1Certificate certifyC1(IPoincareMap& pmPeri)
{
  ...
  C.gLeft  = gAtPoint(pmPeri,L);
  C.gRight = gAtPoint(pmPeri,U);
  C.gPrime = gPrimeOn(pmPeri,interval(L,U));
  ...
  if(!(strictlyNegative(C.gLeft) && strictlyPositive(C.gRight) && strictlyPositive(C.gPrime))){
    cout << "C1 FAIL: sign/monotonicity bracket did not validate.\n";
    return C;
  }

  // Rigorous bisection: only update when CAPD proves a strict sign.
  for(int k=0;k<42;++k){
    double m=0.5*(L+U);
    interval gm=gAtPoint(pmPeri,m);
    if(strictlyNegative(gm)) L=m;
  ...
  // Hyperbolicity of F=P^4 on the full root enclosure.
  MapEval F = evalP(pmPeri,C.pRoot,ZERO,4);
  C.DF=F.D2;
